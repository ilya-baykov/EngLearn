...
urlpatterns = [
    ...
    path('<slug:word_slug>/add_examples/', views_cart_edit.AddWordExample.as_view(), name='add_examples'),
    path('<slug:word_slug>/change_image/', views_cart_edit.change_image, name='change_image'),
    path('<int:example_id>/<slug:word_slug>/remove_example/', views_cart_edit.remove_current_example,
         name='remove_current_example'),
]
